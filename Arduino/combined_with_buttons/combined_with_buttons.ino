...
// Red - 1, Blue-2 , green-3, yellow-4, violet-5 , orange-6 
...
long int averageFrontBlack[8] = {2949 , 2607 , 2949 , 2949 , 2936 , 2762 , 2935 , 2714};   
long int averageFrontWhite[8] = {1902  , 1200  , 199  , 1960  , 385  , 230  , 262  , 560};  // 876 ko 1200 kiya 
long int averageBackBlack[8] = {2953 , 2953 , 2954 , 2957 , 2948 , 2504 , 2950 , 2947};
long int averageBackWhite[8] = {896 , 232 , 493 , 221 , 911 , 480 , 1498 , 1657   };      
...
void assign_button_directions(){
  ...
  for(int i=0;i<6;i++)
     if(bit_is_clear(PING,1) && buttonreset==1){
      
      while(bit_is_clear(PING,1));
      
      }
      clearAll();
      else if (bit_is_clear(PING,2) && buttonreset ==1 ){

      while(bit_is_clear(PING,2));
      }
      clearAll();
      else if (bit_is_clear(PING,3) && buttonreset==1){
     
     
      while(bit_is_clear(PING,3));
      }
      clearAll();
      else if(bit_is_clear(PING,5) && buttonreset == 1){
      ...
      while(bit_is_clear(PING,5));  
      }
      else if (bit_is_clear(PING,6)) && buttonreset == 1 {

        
      while(bit_is_clear(PING,6));  
      }
      clearAll();
...
ros::Publisher pub2("AtmegaOut", &buttonBlock);
